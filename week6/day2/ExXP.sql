...
--Assuming this is for rental_rate:
SELECT * FROM film
ORDER BY rental_rate
LIMIT 10
--If it is for replacement_cost:
SELECT * FROM film
...
--Sounds like customer_id should not be retrieved.
...
--In case the user does want customer_id as well:
SELECT c.first_name, c.last_name, 
...
--Assuming the user also wants to see that staff member id:
